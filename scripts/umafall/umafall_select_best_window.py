...
project.log("=====================UMAFALL_SELECT_BEST_WINDOWS=====================")
for w in range(10,110,10):
    print("Load data with window len = {}".format(w))
    data_list_people = umafall.load_training_data_from_list_people(w, [p], remove_outliers=0.05, additional_where="and sensor=2 and body=3")
    ...
    y2 = pd.DataFrame(y2, columns=[umafall.label_tag])
...
project.log("===============================================================")
#base_classification = Base_Classification(umafall, ExtraTreesClassifier(n_estimators = 1000, random_state=0)) # Best algoritm
