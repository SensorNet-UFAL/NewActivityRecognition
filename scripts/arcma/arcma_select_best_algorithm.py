...
for c in classifiers:
    ...
    for p in persons:
        s = save()
        try:
            relevant_features = s.load_var("arcma_relevante_features_fix_window{}relevant_features_{}.pkl".format(slash, p))
            y = s.load_var("arcma_relevante_features_fix_window{}y_{}.pkl".format(slash, p))
    ...
    project.log("Classifier = {} | Accuracy = {}".format(type(classifiers[c]).__name__, st.mean(person_accuracies)), file="arcma_best_algorithm.log")
    break
